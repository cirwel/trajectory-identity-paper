...
**The observation channel: process-traces versus product-traces.** Recognition as defined here operates on a signature $\Sigma_B$ computed from a *trajectory* — a *process-trace*, a time-indexed series of $B$'s state (the governance EISV+φ series of §6.5, or the anima sensorimotor series of §6.4). This differs from the *product-traces* an agent more commonly encounters in a shared workspace: static artifacts — committed code, comments, documents — that carry no time-indexed state. The discrimination evidence of §6.5 is established on process-traces and does not, on its own, establish that static product-traces carry a comparable individuating signal. A small within-model pilot bearing on this question (effort-level variants of a single model, surface-text features, $n \le 2$ per cell; `experiments/cross-llm-discrimination`) found such features information-poor — between-agent distance fell *below* the within-agent stochastic variance (ratio $0.87$) — but it is underpowered and within-model, so it is suggestive rather than conclusive. The honest scope is therefore that inter-agent recognition, as evidenced here, presumes access to an instrumented process-trace; recognition from static artifacts alone — an agent inferring authorship from another's committed work — is a weaker and largely open problem. This is consistent with the framework's structure: the components that carry the recognition signal ($\alpha$, $\rho$; §5.5.2) are by construction properties of *dynamics*, not of any single static product. Definition 5.4 is thus bounded to deployments where $B$'s trajectory is observable, and authorship-attribution-from-artifacts is flagged as future work (§7.2).
