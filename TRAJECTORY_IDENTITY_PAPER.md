...
**Recognition presumes an observable trajectory.** The signature $\Sigma_B$ is computed from a *process-trace* — a time-indexed series of $B$'s state (the governance EISV+φ series of §6.5, or the anima sensorimotor series of §6.4) — not from the static artifacts (committed code, documents) an agent typically encounters in a shared workspace. Inferring an author from such static products is a distinct and largely open problem: the components carrying the recognition signal ($\alpha$, $\rho$; §5.5.2) are properties of *dynamics*, not of any single product. Definition 5.4 is therefore bounded to deployments where $B$'s trajectory is observable (§7.2).
...
- `src/anima_mcp/trajectory.py` — composite signature, the §4.1 weighted similarity (**see the correction below: it did not implement §4.1 before 2026-08-14**), and the two-tier continuity / anomaly-detection logic of §4.3.

> **Correction (v0.16, 2026-08-14) — this appendix misdescribed the reference implementation, and the implementation has since been corrected.** From v0.13 through v0.15 the appendix stated that the reference implementation "tracks the v0.12 spec," using the five informationally-independent weights $(0.18, 0.18, 0.30, 0.22, 0.12)$ for $(\Pi, \beta, \alpha, \rho, \Delta)$ with $\eta$ exposed as a derived view rather than included in the weighted sum. **That statement was incorrect for the whole of that period.** The `similarity()` method in `src/anima_mcp/trajectory.py` implemented the pre-v0.11 six-component sum:
>
...
> | As corrected | 0.18 | 0.18 | 0.30 | 0.22 | 0.12 | — |
>
> An em dash marks $\eta$'s absence from the weighted sum, where §3.6 places it as a derived view. The middle row is what shipped from 2026-04-03 to 2026-08-14, carrying $\eta$ as a full sixth term; the last row is the state from 2026-08-14 onward.
>
> The $\eta$ double-counting that §3.6 identifies was therefore present in the code, in a sharper form than §3.6 assumes: `compute_trajectory_signature()` *constructs* $\eta$ by aliasing the other components, assigning `set_point` from `attractor["center"]`, `basin_shape` from `attractor["covariance"]`, and `recovery_tau` from `recovery["tau_estimate"]`. The $\eta$ term was thus a re-weighting of $\alpha$ and $\rho$ rather than an independent measurement channel — precisely the reason §3.6 demoted it.
>
> **How the error arose.** The v0.12 revision corrected a Python listing that was at the time embedded in this appendix, and recorded that "the reference implementation now matches the prose." That was true of the listing. v0.13 then dropped the listing for journal word count and replaced it with the repository pointer above, carrying the "tracks the v0.12 spec" sentence across to a repository that had never been changed. The cited code was last revised for this purpose on 2026-04-03 (`ef05b77`, "Implement full six-component trajectory signature from paper") — five weeks before v0.11 demoted $\eta$. A claim true of one artifact had silently re-attached to a different artifact during an unrelated edit.
>
> **Remedy.** `anima-mcp` was brought into line on 2026-08-14 (commit `3ac59e6`). The five §4.1 weights now live in a single `SIMILARITY_WEIGHTS` constant used by both the fixed and the adaptive weighting paths; $\eta$ is still computed and reported, under a `derived` key, but carries no weight and no longer enters the component history that the inverse-variance scheme learns from. Two terminology items that had travelled the same route were corrected with it: `is_same_identity()` is now `is_operationally_continuous()` (retaining the old name as a delegating alias) per §4.3, and the `similarity()` docstring no longer glosses $\mathrm{sim}(\Sigma_1, \Sigma_2) > \theta$ as "same identity."
>
> **Scope.** This was a defect in the appendix, not in the framework. §3.6 and §4.1 are unchanged and the five-weight specification remains the paper's claim. No empirical result depends on the linked code: §6.4 and §6.5 are computed by the standalone analysis code in `scripts/` and `experiments/resident-discrimination/`, neither of which imports `anima_mcp`. The correction is also inert for the one long-running deployment reported here: Lumen's genesis signature was frozen before $\eta$ was computable and stores it as null, so the $\eta$ term could never fire against genesis, and its lineage similarity is unchanged to twelve decimal places.

**Where the two disagree, §3.6 and §4.1 govern.** The specification is the five informationally-independent weights $(0.18, 0.18, 0.30, 0.22, 0.12)$ for $(\Pi, \beta, \alpha, \rho, \Delta)$ with $\eta$ exposed as a derived view. The reference implementation now agrees, but it is an illustration of one embodiment — component extraction, genesis persistence, the shape of two-tier detection — and implementers porting to another platform should take the paper as normative.
...
- **Appendix A corrected.** From v0.13 through v0.15 the appendix asserted that the linked reference implementation (`anima-mcp`) tracked the v0.12 spec — five informationally-independent weights with $\eta$ held out of the weighted sum. It did not, and never had: `similarity()` implemented the pre-v0.11 six-component sum with $\eta$ inside it at weight 0.15, over weights $(0.15, 0.15, 0.25, 0.20, 0.10)$. The appendix now states the implemented weights, records how the false claim propagated (v0.12 corrected an embedded listing; v0.13 replaced that listing with a repository pointer and carried the "tracks the v0.12 spec" sentence across to code that had never changed), and makes the specification governing where the two disagree.
- **The implementation was corrected too**, on 2026-08-14 (`anima-mcp` commit `3ac59e6`), so the appendix's description now holds going forward: the five §4.1 weights live in one `SIMILARITY_WEIGHTS` constant, $\eta$ is reported as a derived view but never weighted and never enters the inverse-variance component history, and `is_same_identity()` becomes `is_operationally_continuous()` per §4.3. The change is inert for the deployment reported in §6.4 — Lumen's genesis stores $\eta$ as null, so the term could never fire against genesis and lineage similarity is unchanged to twelve decimal places.
